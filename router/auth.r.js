...
router
  ...
  .post(
    ...
    validator.singUpValidator,
    validatorHandler("/auth/register"),
    auth.postRegister,
  );
...
router
  ...
  .post(
    ...
    validatorHandler("/auth/login"),
    auth.loginPost,
  );

router.route("/logout").get(middlewares.isAuth, auth.logOutGet);
