...
exports.getReset = function(req, res, next) {
  const msgObj = errorMessage(req.flash('errors')) ?? messageRawList(req.flash('success'));
  const contex = {
    msgObj,
...
exports.forgetPasswordGet = async function(req, res, next) {
  ...
  const resetPassToken = await tables.resetPassword.findOne({ where: { token } });
...
exports.forgetPasswordPost = async function(req, res) {
  ...
  if (!user) {
    req.flash('errors', [{ msg: "مشکلی پیش اومده لطفا بعدا دوباره تلاش کنید" }])
    return redirect(req, res, 400, '/auth/reset')
  }
