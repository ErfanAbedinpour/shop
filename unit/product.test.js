...
describe("render pages succes", function () {
  ...
  it("faild to render must be send error", async function () {
    const findAllMock = jest
      .spyOn(tables.Category, "findAll")
      .mockImplementationOnce(() => Promise.reject("faild to fetch datas"));
...
describe("product login test", function () {
  it("create product", async function () {
    const mockProductFn = jest
      .spyOn(tables.Product, "create")
    ...
    expect(mockReq.files.product.concat).toHaveBeenCalledWith(
      mockReq.files.title,
    );
  ...
  it("error when create product", async function () {
    const mockProductFn = jest
      .spyOn(tables.Product, "create")
      .mockImplementationOnce((product) =>
        Promise.reject("faild to create user"),
      );
    await productController.createProduct(mockReq, mockRes, next);
  ...
  it("should be return error when product id in invalid", async function () {
    ...
    expect(mockReq.flash).toHaveBeenCalledWith("errors", [
      { msg: "درخواست معتبر نیست", color: "red" },
    ]);
  });
  ...
  it("should be return false when cannot find product in DB", async function () {
    ...
    expect(mockReq.flash).toHaveBeenCalledWith("errors", [
      { msg: "کالا پیدا نشد", color: "red" },
    ]);
  });

  it("should be delete product ", async function () {
    const findOneMock = jest
      .spyOn(tables.Product, "findOne")
      .mockReturnValueOnce({
        ...product,
        destroy: jest.fn().mockResolvedValue(),
        save: jest.fn().mockResolvedValue(),
        productImage: [{ fileanme: "image.jpg" }, { fileanme: "iamge2.jpg" }],
      });
